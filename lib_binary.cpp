...
CBL::ADDRESS CBL::find_by_signature(const std::vector<CBL::BYTE> &buffer, const std::vector<CBL::BYTE> &signature, CBL::ADDRESS start_index){
...
std::vector<CBL::ADDRESS> CBL::find_all_by_signature(const std::vector<CBL::BYTE> &buffer, const std::vector<CBL::BYTE> &signature, CBL::ADDRESS start_index){
...
uint8_t CBL::buf2u8(const std::vector<CBL::BYTE> &buffer, ADDRESS offset){
    return buffer.size() > offset ? buffer[offset] : 0;   
}
uint16_t CBL::buf2u16(const std::vector<CBL::BYTE> &buffer, ADDRESS offset){
    uint16_t result = {0};
...
uint32_t CBL::buf2u32(const std::vector<CBL::BYTE> &buffer, ADDRESS offset){
    uint32_t result = {0};
    char size = buffer.size()-offset <= 4 ? buffer.size()-offset : 4;
    for (unsigned int i = 0; i < size ; ++i) result += buffer[offset+size-i-1]*std::pow(0x100, i);
    return result;
}
uint64_t CBL::buf2u64(const std::vector<CBL::BYTE> &buffer, ADDRESS offset){
    uint64_t result = {0};
